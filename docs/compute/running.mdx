...
```bash
# Check node configuration and attestation
blindference-node status

# List recent completed jobs with earnings
blindference-node jobs list --limit 10

# Total BLIND earned across all jobs
blindference-node jobs earnings

# Check on-chain stake, consecutive failures, slash risk
blindference-node staking status

# BLIND token balance in your wallet
