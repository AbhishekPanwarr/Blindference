...
<Tabs>
  <Tab title="GitHub Clone (Recommended)">
    ...
    This is the recommended path. The CoFHE bridge requires Node.js packages that cannot be bundled in a Python wheel.
  </Tab>
  <Tab title="PyPI (Advanced)">
    <Warning>
      The PyPI wheel includes Python code but **NOT Node.js dependencies**. After `pip install`, you **must** run `npm install` inside the installed package directory.
    </Warning>
    ...
    ```bash title="Install Node.js dependencies (REQUIRED)"
