...
async def get_assignments(
    ...
    for task_id in task_ids:
        ...
        assignments.append({
            "jobId": str(task_id),
            "role": role,
            "modelId": request_doc.get("model_id", "qwen2.5-7b") if isinstance(request_doc, dict) else "qwen2.5-7b",
            "promptCid": request_doc.get("prompt_cid", "") if isinstance(request_doc, dict) else "",
            "deadline": int(time.time()) + 600,
            "insuranceOptIn": False,
            "userAddress": request_doc.get("developer_address", "0x0000000000000000000000000000000000000000") if isinstance(request_doc, dict) else "0x0000000000000000000000000000000000000000",
        })
...
async def claim_task(
    ...
    return {
        "kpHighHandle": int(handles.get("high", "0")),
        "kpLowHandle": int(handles.get("low", "0")),
