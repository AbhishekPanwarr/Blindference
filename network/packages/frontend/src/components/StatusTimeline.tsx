...
export function StatusTimeline({ currentStatus, timestamps }: { currentStatus: string; timestamps: Record<string, number> }) {
  ...
  return (
    <div className="flex items-center justify-between w-full relative mb-10 mt-4 px-2">
      <div className="absolute top-3.5 left-0 w-full h-[2px] bg-violet-500/10 -z-10 rounded-full" />
      <div className="absolute top-3.5 left-0 h-[2px] bg-violet-500/40 -z-10 rounded-full transition-all duration-1000" style={{ width: `${Math.max(0, currentIndex) / (stages.length - 1) * 100}%` }} />
        ...
        return (
          <div key={stage} className="flex flex-col items-center relative group">
            <div className={`w-7 h-7 rounded-full flex items-center justify-center border-2 transition-all shadow-sm ${
                isCompleted
                  ? 'bg-violet-500/20 border-violet-500 text-violet-400 shadow-glow-sm'
                  : isRejected
                     ? 'bg-error/20 border-error text-error'
                     : 'bg-[rgba(10,10,10,0.8)] border-violet-500/20 text-white/50'
               }`}>
              {isCompleted ? <CheckCircle2 className="w-3.5 h-3.5" /> : <div className={`w-1.5 h-1.5 rounded-full ${isCurrent ? 'bg-violet-400 animate-glow-pulse' : 'bg-transparent'}`} />}
            </div>
