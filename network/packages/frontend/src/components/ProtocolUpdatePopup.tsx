      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
          ...
          <motion.div
            ...
            className="relative w-full max-w-4xl overflow-hidden rounded-2xl bg-[#0d0d0d] border border-zinc-800 shadow-[0_0_40px_rgba(255,255,255,0.05)] max-h-[90vh] overflow-y-auto"
          >
            <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-zinc-500 to-white" />

            <button
              onClick={closePopup}
              className="absolute right-4 top-4 rounded-full p-2 text-zinc-500 hover:bg-zinc-800 hover:text-white transition-colors z-10"
            >
              <X className="h-5 w-5" />
            </button>

            <div className="p-8 sm:p-12">
              <div className="mb-6 inline-flex items-center gap-2 rounded border border-zinc-700 bg-zinc-800 px-3 py-1.5 text-[10px] font-bold uppercase tracking-widest text-zinc-300">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
                {[
                  ...
                  {
                    icon: Monitor,
                    title: '5. Pure Black Protocol Aesthetic',
                    desc: 'A massive frontend overhaul bringing a sleek, developer-focused dark mode experience. Fluid framer-motion micro-animations and intuitive workflows tailored for web3 engineers.',
                  },
                ].map((feature) => {
                  ...
                  return (
                    <div
                      key={feature.title}
                      className="flex items-start gap-4 p-4 rounded-xl border border-zinc-800 bg-zinc-900/40 hover:bg-zinc-900/60 transition-colors"
                    >
                      <div className="rounded border border-zinc-700 bg-zinc-800 p-3 mt-1 text-zinc-400">
              ...
              <div className="flex justify-center">
                <button
                  onClick={closePopup}
                  className="w-full sm:w-auto min-w-[300px] rounded bg-white hover:bg-zinc-200 px-8 py-4 text-sm font-bold uppercase tracking-widest text-black transition-colors shadow-[0_0_20px_rgba(255,255,255,0.1)]"
                >
