...
```json title="~/.blindference/config.json"
{
  "node_address": "0x...",
...
<Tabs>
  <Tab title="Bridge Mode (Default)">
    ...
    ```bash title="Run in bridge mode"
    BLF_COFHE_MODE=bridge blindference-node run
    ```

    <Tip>
      Bridge mode is recommended for production. It handles CoFHE SDK initialization more reliably than direct HTTP calls.
    </Tip>
  </Tab>
  <Tab title="Python Mode (Alternative)">
...
```bash title="Set multiple supported models"
...
```bash title="Non-interactive environment setup"
...
```bash title="Local development configuration"
# ICL running locally
...
<Warning>
  Never commit `.env` files with private keys or API keys to version control. Use a secrets manager in production.
</Warning>
