...
**Your Machine → Network → Quorum**

1. Your prompt is AES-256-GCM encrypted locally
2. The AES key is split and CoFHE-encrypted
3. Encrypted blob is uploaded to IPFS
4. Job is submitted to Payment Service
5. ICL selects 1 Leader + 2 Verifiers
6. Nodes decrypt, run inference, submit hashes
7. 2/3 consensus required for acceptance
8. Result is encrypted and returned to you
9. You decrypt locally — plaintext never touches a server
...
- [Quickstart](quickstart) — Run your first confidential inference in 5 minutes
- [CLI Reference](cli-reference) — All commands and options
- [SDK API](sdk-api) — Programmatic usage with TypeScript
- [Configuration](configuration) — Environment variables and advanced options
